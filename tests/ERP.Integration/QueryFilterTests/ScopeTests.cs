...
public class ScopeTests : NorthwestIntegrationTestBase
{
    ...
    {
        RequestContextService srv = GetService<RequestContextService>();
    ...
    public void GivenRequestContext_WhenCreatedWithScopeNorthwestIntegrationBase_ThenRequestContextServiceExists()
    {
        ...
        RequestContextService requestContextService = scope.ServiceProvider.GetRequiredService<RequestContextService>();
    }
    ...
    public async Task GivenNorthwestContext_WhenQueryPlayer_ThenProvidesUserId()
    {
        ...
        using (var scope1 = _serviceProvider.CreateScope())
        {
            RequestContextService requestContextService =
                scope1.ServiceProvider.GetRequiredService<RequestContextService>();
            IMediator mediatorScoped = scope1.ServiceProvider.GetRequiredService<IMediator>();
        ...
        using (var scope1 = _serviceProvider.CreateScope())
        {
            RequestContextService sc = scope1.ServiceProvider.GetRequiredService<RequestContextService>();
    ...
    [Fact]
    public async Task GivenNorthwestContext_WhenQueryPlayer_ThenProvidesUserId2()
    {
        Guid res = await Mediator.Send(new RegisterRequest()
        {
            Username = "Fred",
            Password = "123"
        });

        LoginResult res2 = await Mediator.Send(new LoginRequest()
        {
            Username = "Fred",
            Password = "123"
        });

        RequestContextService requestContextService = base.GetService<RequestContextService>();


        // From now on, the user is logged in.
        requestContextService.UserId = res2.UserId;
